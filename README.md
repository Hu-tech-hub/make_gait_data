...
# 가상환경 생성 (권장)
python -m venv vision_gait

# 가상환경 활성화
# Windows:
vision_gait\Scripts\activate
# macOS/Linux:
source vision_gait/bin/activate
...
#### 🔍 Step 3: 보행 이벤트 검출
- 발목 y좌표에서 피크 검출 알고리즘 적용
- HS(Heel Strike): 발목 최저점
- TO(Toe Off): 발목 최고점
...
분석 완료 후 `./gait_analysis_output/` 디렉토리에 다음 파일들이 생성됩니다:
...
| `gait_events_plot.png` | 이벤트 검출 시각화 그래프 |
...
### 보행 이벤트
- **HS (Heel Strike)**: 발뒤꿈치가 지면에 닿는 순간
- **TO (Toe Off)**: 발가락이 지면에서 떨어지는 순간
